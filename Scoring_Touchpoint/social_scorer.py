...
for f_i,f_result in enumerate(f_cursor):
    f_data[f_i] = {}
    try:
        f_no['l'].append(len(f_result['likes']['data']))
    except:
        f_no['l'].append(0)
    try:
        f_no['c'].append(len(f_result['comments']))
    except:
        f_no['c'].append(0)
    try:
        f_no['s'].append((f_result['shares']['count']))
    except:
        f_no['s'].append(0)
    f_data[f_i] = f_result
...
for y_i,y_result in enumerate(y_cursor):
    y_data[y_i] = {}
    try:
        y_no['l'].append(len(y_result['likes']['data']))
    except:
        y_no['l'].append(0)
    try:
        y_no['c'].append(len(y_result['comments']))
    except:
        y_no['c'].append(0)
    try:
        y_no['s'].append((y_result['shares']['count']))
    except:
        y_no['s'].append(0)
    y_data[y_i] = y_result
